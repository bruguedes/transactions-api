defmodule Transactions.ViaCep.ClientTest do
  use ExUnit.Case

  alias Transactions.ViaCep.Client
  ...
  describe "get_cep_info/1" do
    ...
    test "when there is a invalid cep, returns an error", %{bypass: bypass} do
      ...
      expected_response =
        {:error, %Transactions.BuildError{result: "invalid CEP", status: :bad_request}}
    ...
    test "when the cep was not found, returns an error", %{bypass: bypass} do
      ...
      expected_response =
        {:error, %Transactions.BuildError{result: "CEP not found!", status: :not_found}}
    ...
    test "when there is as generic error , returns an error", %{bypass: bypass} do
      ...
      expected_response =
        {:error, %Transactions.BuildError{result: :econnrefused, status: :bad_request}}
